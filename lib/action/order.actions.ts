...
export async function getOrderBySummary() {
  ...
  // Get monthly sales
  const salesDataRaw = await prisma.$queryRaw<Array<{month: string; totalSales: Prisma.Decimal}>>`SELECT to_char("createdAt", 'MM/YY') as "MONTH", sum("totalPrice") as "totalSales" FROM "Order" GROUP BY to_char("craetedAt", "MM/YY")`;
